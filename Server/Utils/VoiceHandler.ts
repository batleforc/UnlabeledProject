...
class VoiceHandler {
  ...
  DeleteSong = (io: Server, id: number) => {
    this.songQueue.queue.splice(id, 1);
    io.emit("VoiceQueue");
  };

  Play = async (io: Server, song?: Song, now?: boolean) => {
    if (!this.songQueue.canPlay || !song) return;
    if (song.type === 2)
      song.title = (await ytdl.getInfo(song.url)).videoDetails.title;
    if (now) {
      this.songQueue.queue.splice(1, 0, song);
      this.songQueue.connection?.dispatcher?.end();
    } else this.songQueue.queue.push(song);
    io.emit("VoiceQueue");
