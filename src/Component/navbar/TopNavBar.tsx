...
export const TopNavBar = ({ Bot, dispatch, ShowModal, ...props }: any) => {
  return (
    <nav className="bg-secondary text-white flex">
      ...
      <div
        id="BotLogo"
        className="flex flex-1 place-items-center text-center justify-end leading-3"
      >
        {Bot.link !== "" && (
          <div className="px-2">
            <Link to="/token">
      ...
      <svg
        ...
        onClick={() => dispatch(setBurger(!ShowModal.Burger))}
        className="rounded-md z-10 w-9 hover:bg-gray-200 mx-0.5"
        viewBox="0 0 20 20"
        fill="currentColor"
      >
        <path
